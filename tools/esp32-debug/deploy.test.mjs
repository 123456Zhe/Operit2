...
import {defaultProject} from './default-project.mjs';
import {deployRoute,nextLayoutSlot} from './deploy-api.mjs';
import {aiRoute,previewCodeEdits} from './ai-api.mjs';
...
test('independent AI tool loop reads source, handles split UTF8 and validates operations',async t=>{
 ...
 const message=calls===1?{role:'assistant',content:null,tool_calls:[{id:'source',type:'function',function:{name:'read_source',arguments:JSON.stringify({path:'tools/esp32-debug/routes.mjs'})}}]}:{role:'assistant',content:JSON.stringify({summary:'修改首页按钮',operations:[{op:'update',id:'home_apps',changes:{text:'Open'}}]})};
 ...
 await assert.rejects(previewCodeEdits([{path:'tools/esp32-debug/routes.mjs',revision:'stale',find:'anything',replace:''}]),/变化/);
});
