...
```powershell
node tools/esp32-debug/agent.mjs ui_components
node tools/esp32-debug/agent.mjs ui_read
node tools/esp32-debug/agent.mjs ui_patch @patch.json
node tools/esp32-debug/agent.mjs ui_build_status
```
...
{
  "command":"node",
  "args":["D:/game/Operit2/tools/esp32-debug/agent.mjs","--mcp"],
...
保存是修改真实 `apps/esp32/ui/layout.json` 源文件，不自动改写 C，也不启动编译。`package-layout.mjs` 生成设备 OUI2 数据包，普通用户通过网页下发，或 POST `/api/deploy/layout`。只有新硬件能力才修改 C/Rust 并显式构建基础运行时。部署协议详见 FRONTEND.md。共享运行时支持的数据约束不可在网页单独放宽。
