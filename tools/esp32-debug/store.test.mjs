...
test('actual firmware C store validates and commits packages with interruption recovery',t=>{
 ...
 const source=fileURLToPath(new URL('./tests/layout-store.c',import.meta.url)),exe=path.join(dir,'store.exe'),packet=path.join(dir,'layout.oui');writeFileSync(packet,packLayout(defaultProject()));
