...
普通使用只需 Node.js 18+ 和已有的 `generated/ui.mjs`、`ui.wasm`、`manifest.json`，没有 npm 依赖，也无需安装编译器：

```powershell
node tools/esp32-debug/server.mjs
```
...
```powershell
python tools/esp32-debug/build.py --firmware
```

构建需要 Python、Emscripten 4.0.14、Rust/ESP-IDF 工具链。Emscripten 默认 `C:/t/operit-emsdk`；可设置 `EMSDK` 或传 `--emsdk`。LVGL 默认复用 ESP32 构建目录 v9.3.0，可传 `--lvgl`。仅保留当前增量对象与当前产物。
...
- `build.py`：编译 LVGL 和共用 UI、生成 Wasm、可同时构建 ESP32，记录产物哈希。
- `wasm/bridge.c`：RGB565 帧缓冲、触摸、LVGL 内存统计和动作回调。
- `wasm/esp_timer.h`：浏览器时钟适配。
- `app.js`：WebAssembly 宿主和调试面板，不包含按钮布局绘制逻辑。
- `server.mjs`：本地服务、运行时状态、布局/AI/部署接口。
...
开发者新增控件/底层能力才运行 `python tools/esp32-debug/build.py --firmware`。编译保留单份增量对象；页面修改不会触发构建。设备布局分区位于 `0x3E0000`，64 KiB，两个32 KiB槽位，单包上限28 KiB；CRC/尺寸/层级/复杂度验证通过才写入提交头，当前页面在 LVGL 主循环安全切换。失败或中断保留上一份。读取使用 Flash 映射，上传缓冲512字节，不把整份 JSON/页面树加载到堆。
...
路由描述见 `routes.mjs`，共用执行逻辑在 `operit_lvgl.c`。新页面只需编辑项目数据；新设备能力需要修改共享 C/Rust 并更新基础运行时。软件聊天桥接规范见 [FRONTEND.md](FRONTEND.md)，AI 回复示例见 [AGENT_API.md](AGENT_API.md)。
