// Shared capability list for the editor, validation and Agents.
// Every route must have a matching implementation in operit_lvgl.c.
export const routes = [
...
export const actions = routes.map(route => route.id);
export const eventBindings = [
  {field:'action', event:'click', label:'点击'},
  {field:'longAction', event:'long_press', label:'长按'},
];
export const routeLabel = id => routes.find(route => route.id === id)?.label || id;
