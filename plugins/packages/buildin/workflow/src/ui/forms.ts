...
import { copy, type ToolDefinition, type ToolParameterSchema, type Workflow, type WorkflowNode, type Value, type Trigger } from "../model";
...
function toolParameterField(ctx: ComposeDslContext, key: string, schema: ToolParameterSchema, value: Value, workflow: Workflow, self: string, change: (value: Value) => void): ComposeNode {
  ...
  const referenceToggle = ctx.UI.Row({ verticalAlignment: "center", spacing: 8 }, [
    ...
    ctx.UI.Switch({ checked: referenced, onCheckedChange: checked => {
      if (checked) {
        ...
        if (source === undefined) { void ctx.showToast("请先创建上游节点"); return; }
        change({ nodeId: source.id });
      } else change({ value: schema.default ?? "" });
    } }),
...
export function nodeForm(ctx: ComposeDslContext, workflow: Workflow, node: WorkflowNode, tools: ToolDefinition[], change: (node: WorkflowNode) => void, configureSchedule: () => void): ComposeNode[] {
  const UI = ctx.UI;
  ...
  if (node.type === "execute") {
    ...
    content.push(choose(ctx, `${node.id}:tools`, "执行工具", node.actionType, orderedTools.map(tool => ({ value: tool.name, label: tool.name })), actionType => {
      ...
      if (tool === undefined) throw new Error(`工具元数据不存在：${actionType}`);
    ...
    if (selectedTool?.description) content.push(UI.Text({ text: selectedTool.description, style: "bodySmall", color: "onSurfaceVariant" }));
    if (selectedTool) {
      for (const schema of selectedTool.parameters) {
        content.push(toolParameterField(ctx, `${node.id}:param:${schema.name}`, schema, node.actionConfig[schema.name] ?? { value: schema.default ?? "" }, workflow, node.id,
          next => change({ ...node, actionConfig: { ...node.actionConfig, [schema.name]: next } })));
      }
      if (selectedTool.parameters.length === 0) content.push(UI.Text({ text: "此工具不需要参数", style: "bodySmall", color: "onSurfaceVariant" }));
    }
  ...
  if (node.type === "logic") content.push(choose(ctx, `${node.id}:logic`, "逻辑运算", node.operator, [{ value: "AND", label: "且" }, { value: "OR", label: "或" }], operator => change(parseNode({ ...node, operator }))), UI.Text({ text: "对输入连线中成功完成的布尔结果运算。", style: "bodySmall" }));
